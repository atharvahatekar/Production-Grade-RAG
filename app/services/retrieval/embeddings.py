...
BATCH_SIZE = 50
_GEMINI_DIM = 3072   # Primary model for embeddings
...
def _probe_gemini():
    ...
    try:
        model = GoogleGenerativeAIEmbeddings(
            ...
            credentials_path=settings.GEMINI_API_KEY,
        )
        # Test call to verify connectivity
        model.embed_query("Test Probe")
        logfire.info("Gemini embeddings ready (gemini-embedding-2-preview, 3072-dim).")
        return model
    except Exception as e:
        logfire.warning(f"Gemini Try Probe failed: {e}. Will use sentence-transformers fallback.")
        return None
